...
public class FlowblazeManager implements FlowblazeService {
    ...
    private boolean resetFlowRules(TableId tableId) {
        ...
        if (flowRules.size() == 0) {
            log.debug(String.format("NO rule to reset on %s Table", tableId));
            return true;
        }
